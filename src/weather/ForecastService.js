...
export class ForecastService {
  ...
  transformToDailyForecast(rawForecastResponse) {
    ...
    const timezoneOffsetInSeconds = typeof rawForecastResponse.city?.timezone === 'number'
      ? rawForecastResponse.city.timezone
      : 0;

    const groupedDaysMap = new Map();

    for (let index = 0; index < rawForecastResponse.list.length; index += 1) {
      const forecastItem = rawForecastResponse.list[index];

      if (typeof forecastItem.dt !== 'number' || typeof forecastItem.main?.temp !== 'number') {
        continue;
      }

      // Convert UTC timestamp to local timestamp using the city's timezone offset
      const localTimestampMilliseconds = (forecastItem.dt + timezoneOffsetInSeconds) * 1000;
      const localDateRepresentation = new Date(localTimestampMilliseconds);
      ...
      groupedDaysMap.get(dayKey).items.push({
        ...
        localHour: localHour,
        condition: forecastItem.weather && forecastItem.weather.length > 0 ? forecastItem.weather[0].main : 'Unknown',
        weatherIcon: forecastItem.weather && forecastItem.weather.length > 0 ? forecastItem.weather[0].icon : '01d'
      });
    }
    ...
    for (const [, dayGroup] of groupedDaysMap.entries()) {
      ...
      for (let itemIndex = 0; itemIndex < dayGroup.items.length; itemIndex += 1) {
      ...
      const calendarDate = new Date(dayGroup.localYear, dayGroup.localMonth, dayGroup.localDay, 12, 0, 0);
