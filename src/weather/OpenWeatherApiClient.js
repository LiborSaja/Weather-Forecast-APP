...
export class OpenWeatherApiClient {
    ...
    async fetchForecast({ latitude, longitude, signal = undefined }) {
        ...
        if (typeof latitude !== "number" || typeof longitude !== "number") {
            throw new Error(
                "Valid latitude and longitude numerical coordinates are required.",
            );
        }
        ...
        if (!response.ok) {
            if (response.status === 401) {
                throw new Error(
                    "Invalid OpenWeather API key. Please check your VITE_OPENWEATHER_API_KEY configuration.",
                );
            } else if (response.status === 404) {
                throw new Error(
                    "Forecast data for the selected location could not be found.",
                );
            } else if (response.status === 429) {
                throw new Error(
                    "OpenWeather API rate limit exceeded. Please try again in a few moments.",
                );
            } else if (response.status >= 500) {
                throw new Error(
                    "OpenWeather service is currently experiencing issues. Please try again later.",
                );
            } else {
                throw new Error(
                    `Weather service responded with error status ${response.status} (${response.statusText}).`,
                );
            }
        }
        ...
        if (
            ...
            throw new Error(
                "Received unexpected or malformed forecast response structure from weather service.",
            );
        }
